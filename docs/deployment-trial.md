...
- **ディスクはエフェメラル**。アップロード画像は再起動・再デプロイのたびに消える。
  DBは `DATABASE_URL` に Turso（無料）を設定すれば永続化される。未設定の場合は
  ローカルSQLiteとなり、クイズは毎回作り直す前提で使う（起動時に `db:migrate` が走る）。
...
3. 環境変数の入力を求められたら:
   ...
   - `DATABASE_URL` / `DATABASE_AUTH_TOKEN`: Turso を使う場合に設定（任意。
     `turso db show <db> --url` と `turso db tokens create <db>` で取得。
     設定するとDBが再起動後も消えなくなる。空のままならエフェメラルなローカルDB）
