...
> Socket.io は Redis 等のアダプタ未使用でプロセスローカルにルームを管理し、SQLite もインスタンスごとの
> ローカルファイルです。インスタンスを増やすとロードバランサの振り分けで参加者リストが分裂し、
> `nextQuestion` が一部の参加者にしか届かず、スコアも別々のDBに保存されて合算されない split-brain に
> なります。増強が必要なときは垂直スケール（上位プラン）で対応してください。
> 水平スケールが本当に必要になった場合は、先に Socket.io Redis アダプタ ＋ 共有DB（Turso 等の
> ホスト型 libSQL）への移行が前提です。**本番当日はインスタンス数を1のまま固定すること。**
