...
## 追補: URL修正後もブラウザだけ画像が表示されない（CORP）

### 症状
URL絶対化の修正デプロイ後も全メディアが表示されない。一方で curl / node fetch では 200。

### 原因
hono `secureHeaders()` の既定が **`Cross-Origin-Resource-Policy: same-origin`** を付与しており、
**ブラウザが別オリジン（SWA）ページへの `<img>` 埋め込みをブロック**していた。
CORPはブラウザのみが強制するため、curl・fetch(cors) の検証では検出できなかった。

### 対応
- `secureHeaders({ crossOriginResourcePolicy: false })` に変更（JSON APIへのCORPは実益が乏しく、
  ミドルウェアがレスポンスヘッダを上書きするため route 側の指定が生きない）
- メディア配信レスポンスに明示的に `Cross-Origin-Resource-Policy: cross-origin` を付与
  （メディアは別オリジンのフロントに埋め込まれることが正規の用途）

### 学び
「curlでは通るのにブラウザで壊れる」ときは CORS だけでなく **CORP / COEP / CSP** を疑う。
検証はブラウザ実機（またはヘッダ確認 `curl -s -D - -o /dev/null <url>`）まで行うこと。
